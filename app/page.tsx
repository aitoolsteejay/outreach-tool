  ...
  return (
    <main className={`shell ${isAdmin ? "adminShell" : ""}`}>
      ...
      <section className="content" id="campaigns">
        ...
        </div> : <><section className="welcome"><div><p className="eyebrow dark">OUTREACH, MANAGED</p><h2>Turn lead lists into<br/>real conversations.</h2><p>Share the right people and your point of view. We’ll handle campaign setup, execution, and reporting.</p><button className="lightButton" onClick={openWizard}>Start a campaign <span>→</span></button></div></section><section className="stats" aria-label="Campaign summary"><div className="statYellow"><span>Active campaigns</span><strong>{workspaceLoading ? "—" : activeCampaigns}</strong><small>In your workspace</small></div><div className="statPurple"><span>Total leads</span><strong>{workspaceLoading ? "—" : totalLeads}</strong><small>Across submitted campaigns</small></div><div className="statGreen"><span>Campaigns submitted</span><strong>{workspaceLoading ? "—" : campaigns.length}</strong><small>All time</small></div></section><section className="campaignSection"><div className="sectionHeading"><div><h3>Your campaigns</h3><p>Track every campaign from brief to replies.</p></div><button className="filter">All campaigns⌄</button></div><div className="campaignList">{campaigns.map((campaign) => <article className="campaign" key={campaign.name}><div className="campaignIcon">↗</div><div className="campaignInfo"><strong>{campaign.name}</strong><span>{campaign.audience} · LinkedIn outreach</span></div><div className="progress"><div><span>Campaign progress</span><b>{campaign.progress}%</b></div><div className="track"><i style={{width:`${campaign.progress}%`}}/></div></div><span className={`status ${campaign.status.replaceAll(" ", "-").toLowerCase()}`}>{campaign.status}</span><button className="more" aria-label={`More options for ${campaign.name}`}>•••</button></article>)}{!workspaceLoading && campaigns.length === 0 && <div className="emptyCampaigns"><span className="emptyIcon">↗</span><strong>No campaigns yet.</strong><p>Create your first campaign when your lead list and messaging direction are ready.</p><button className="primary" onClick={openWizard}>Create campaign</button></div>}</div></section></>}
      </section>
